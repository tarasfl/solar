...
<script lang="ts">
      ...
      if (leads.length > 0) {
        leads.forEach((element: any) => {
          if (
            ...
            element.prospect_name == data.prospect_name
          ) {
  ...
  async function writeData(leadsArray: any[]) {
    update_campaign(leadsArray).then(() =>
      update_lead_campaign(leadsArray).then(() =>
  ...
  async function performWritingData() {
    ...
    if (!buttonClicked) {
      ...
      if (filteredLeads.length > 0) {
        ...
        if (
          confirm('There is no unique leads found, save them anyway? ') == true
        ) {
