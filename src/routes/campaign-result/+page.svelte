...
<script lang="ts">
  ...
  let lastCampaignId =
    campaign[campaign.length - 1] == undefined
      ? 0
      : campaign[campaign.length - 1].campaign_id
  ...
  location = new google.maps.LatLng(
    data[0].center.latitude,
    data[0].center.longitude,
  )
  ...
  function downloadImage() {
    const content = document.getElementById('map')
    if (content) {
      html2canvas(content, { useCORS: true, allowTaint: false }).then(
        (canvas) => {
